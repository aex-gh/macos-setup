...
@test "install-homebrew script exists and is executable" {
    assert_file_exists "${BATS_TEST_DIRNAME}/../scripts/install/install-homebrew.zsh"
    [[ -x "${BATS_TEST_DIRNAME}/../scripts/install/install-homebrew.zsh" ]]
}

@test "install-homebrew detects existing installation" {
    run "${BATS_TEST_DIRNAME}/../scripts/install/install-homebrew.zsh" --check
...
EOF
    ...
    run "${BATS_TEST_DIRNAME}/../scripts/validation/validate-brewfiles.zsh" "${TEST_HOME_DIR}/invalid-Brewfile"
...
@test "install-packages script handles missing Brewfile gracefully" {
    run "${BATS_TEST_DIRNAME}/../scripts/install/install-packages.zsh" --dry-run --device unknown
